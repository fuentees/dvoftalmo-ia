...
assert.equal(dateToEpiWeek(new Date(2026, 0, 1)), 1);
assert.equal(dateToEpiWeek(new Date(2026, 6, 11)), 28);

const saoPauloLateNight = new Date("2026-07-12T02:30:00.000Z");
assert.deepEqual(dateToEpiWeekYear(saoPauloLateNight, "America/Sao_Paulo"), { year: 2026, se: 28 });
assert.deepEqual(currentEpiWeek(saoPauloLateNight), { year: 2026, se: 28 });
assert.equal(currentCalendarYear(saoPauloLateNight), 2026);

const saoPauloSunday = new Date("2026-07-12T15:00:00.000Z");
assert.deepEqual(dateToEpiWeekYear(saoPauloSunday, "America/Sao_Paulo"), { year: 2026, se: 29 });
assert.deepEqual(currentEpiWeek(saoPauloSunday), { year: 2026, se: 29 });
