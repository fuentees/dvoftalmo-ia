-- cevesp_agrupado, grain='month': a coluna "Mes" pode vir nula com frequência
-- nao desprezivel (ja documentado em cevesp_lookup_rpcs.sql, preenchimento
-- <70% em alguns GVEs). Sem fallback, linhas com "Mes" nulo eram excluidas
-- da agregacao mensal (filtro "Mes" >= 1 AND "Mes" <= 12 falha com NULL),
-- subcontando casos mesmo quando DtNotificacao e valida. Aplicado o mesmo
-- padrao ja usado pra SemEpidemio no caminho MySQL direto: coalesce com o
-- mes calculado a partir de DtNotificacao.
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    "ANO"::int AS ano,
    CASE WHEN p_grain = 'month' THEN COALESCE("Mes"::int, EXTRACT(MONTH FROM "DtNotificacao")::int) ELSE NULL END AS mes,
  ...
  WHERE COALESCE("Excluido", 0) = 0
    ...
    AND (p_grain <> 'month' OR (COALESCE("Mes"::int, EXTRACT(MONTH FROM "DtNotificacao")::int) BETWEEN 1 AND 12))
    AND (p_grain <> 'week'  OR ("SemEpidemio" >= 1 AND "SemEpidemio" <= 53))
  GROUP BY
    "ANO"::int,
    CASE WHEN p_grain = 'month' THEN COALESCE("Mes"::int, EXTRACT(MONTH FROM "DtNotificacao")::int) ELSE NULL END,
